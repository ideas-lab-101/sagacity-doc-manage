...
public class VideoController extends WXSSBaseController {
    ...
    public void getClassVideoList() {
        ...
        if (StringTool.notNull(getPara("user_id")) && StringTool.notBlank(getPara("user_id"))) {
            sqlFrom += " and ci.user_id='" + getPara("user_id") + "'";
        }
        if (class_id == -1) {//特殊处理
    ...
    public void getVideoInfo(){
        ...
        //收藏
        video.set("is_favor", false);
        if(StringTool.notNull(getPara("token"))){
            if(Db.findFirst("select * from user_favor where open_id=? and data_id=? and type=?", getPara("token"), videoID, "video") != null){
                video.set("is_favor", true);
            }
        }
